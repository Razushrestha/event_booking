...
import { googleLogin, orgLogin, storeAuthTokens } from "@/services/authServices";
import { signInWithPopup } from "firebase/auth";
import { auth, googleAuthProvider, isFirebaseConfigured } from "@/config/firebase";
import useAuthStore from "@/store/authStore";
import { orgRegister } from "@/services/authServices"; // Import orgRegister
...
export function RegisterOrganization() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await orgRegister(fullName, email, password, phone);

            if (response?.success) {
                notifySuccess("Organization registration successful");
                // router.push("/login")
            } else {
                ...
                <form onSubmit={handleSubmit}>
                    <CardHeader className="space-y-2 pb-6">
                        ...
                        <CardDescription className="text-center text-gray-600">
                            Sign up to start using our Event Solution App for your organization
                        </CardDescription>
                    </CardHeader>
                    ...
                    <CardFooter className="flex flex-col space-y-5 pt-2">
                        ...
                        <Button
                            className="w-full h-11 text-base text-white font-medium bg-gray-600 hover:bg-gray-700 transition-colors"
                            onClick={() => navigate('/')}
                            disabled={isLoading}
                        >
                            Go Back Home
